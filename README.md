...
- 功能升级说明
  - 原来的分类只有一级，2.0开始最高支持三级。
  - 原来只有卡片分类可以设置一个私有分类，2.0开始支持简单角色权限控制，卡片分类通过分配角色控制，卡片也可设置需要登录才能看到的私密信息（目前暂定只做成这样，本项目定位于小型导航服务，基本不会再设计更复杂的权限控制）。
  - 原来的原型卡片类型，更名为静态网站，本质上都是上传一个根目录下有index.html的静态网页压缩包。
  - 原二维码卡片类型去掉，作为普通类型的一个勾选项。
  - 原来只有一个管理员用户，2.0开始拥有用户角色管理，支持个人设置，个人密码设置。
