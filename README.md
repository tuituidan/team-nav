...
- 技术升级说明
  - 前端UI框架由原来的ViewUI改为ElementUI。
  - 数据库由原来的默认模式改为team_nav，不会覆盖1.x的数据库，但需要将数据迁移过来。
- 功能升级说明
  - 1.X的分类只有一级，2.0开始最高支持三级。
  - 1.X只有卡片分类可以设置一个私有分类，2.0开始支持简单角色权限控制，卡片分类通过分配角色控制，卡片也可设置需要登录才能看到的私密信息（目前暂定只做成这样，本项目定位于小型导航服务，基本不会再设计更复杂的权限控制）。
  - 1.X的原型卡片类型，更名为静态网站，本质上都是上传一个根目录下有index.html的静态网页压缩包。
  - 1.X的二维码卡片类型去掉，作为普通类型的一个勾选项。
  - 1.X只有一个管理员用户，2.0开始拥有用户角色管理，支持个人设置，个人密码设置。
  - 1.X可配置不登录就能访问后台，2.0已移除。
  - 1.X可配置是否允许在线修改，2.0已移除，都是个人在线修改自己的密码(管理能重置密码)。
...
通过常规maven命令构建，打包执行命令`mvn clean package -Dmaven.npm.skip=false`，会自动将前端构建到后端的jar包中，这样整个服务就一个jar包部署（当然如果想前后端分离部署也是可以的，这里不再赘述）。
